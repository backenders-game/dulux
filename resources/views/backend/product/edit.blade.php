...
@section('content')
    {{ html()->form('PUT', route('admin.products.update', ['id' => $product->id])
                ...
                <div class="row mt-4 mb-4">
                    <div class="col-md-6 col-sm-12">
                        ...
                        {{ html()->label('<strong>' .__('validation.attributes.backend.products.finish_surface') . '</strong>')
                            ...
                            <div class="col-md-9">
                                <select class="form-control" name="finish_surface_id" value="{{$product->finish_surface_id}}">
                                `   @foreach($finishSurfaces as $finishSurface)
                                    <option value="{{ $finishSurface->id }}" @if($finishSurface->id == $product->finish_surface_id) selected @endif>{{ $finishSurface->name }}</option>
                                    @endforeach
                                </select>
                            </div><!--col-->
                        </div><!--form-group-->
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        {{ html()->label('<strong>' .__('validation.attributes.backend.products.image') . '</strong>')
                            ...
                            <div class="col-md-12" style="text-align:center; height: 16em; margin-top: 1em;">
                                <img style="width: 45%; max-height: 100%;" id="admin_img_product_preview"
                                    src="{{ asset('public/storage/' . $product->img_path) }}" alt="Ảnh sản phẩm" />
                            </div>
                        </div><!--form-group-->

                    </div><!-- end-col-md-6 -->
                    <div class="col-md-12 col-sm-12">
                        @include('backend.product.includes.list_properties', ['properties' => $properties, 'productPrperties' => $productProperties])
                    </div>
                    <div class="col-md-6 col-sm-12">
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-12">
                                {{ html()->textarea('construction_guide')
                                    ...
                                    ->attribute('maxlength', 2000)
                                    ->value($product->construction_guide)
                                    ->attribute('rows', 11) }}
                            </div><!--col-->
                        </div><!--form-group-->
                    </div>
...
@endsection
@push('after-scripts')
{{ script(asset('public/js/ckeditor/ckeditor.js')) }}
