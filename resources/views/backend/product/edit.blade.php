...
@section('content')
    {{ html()->form('POST', route('admin.products.update', $product->id)
                        ...
                        {{ html()->label('<strong>' .__('validation.attributes.backend.products.finish_surface') . '</strong>')
                            ...
                            <div class="col-md-9">
                                ...
                                `   @foreach($finishSurfaces as $finishSurface)
                                    <option value="{{ $finishSurface->id }}" @if($finishSurface->id == $product->finish_surface_id) selected @endif>
                                        {{ $finishSurface->name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        {{ html()->label('<strong>' .__('validation.attributes.backend.products.image') . '</strong>')
                            ...
                            <div class="col-md-12" style="text-align:center; height: 16em; margin-top: 1em;">
                                <img style="width: 45%; max-height: 100%;" id="admin_img_product_preview"
                                    src="{{'http://localhost/dulux/public/storage/' . $product->img_path}}" alt="Ảnh sản phẩm" />
                            </div>
                        </div><!--form-group-->

                    </div><!-- end-col-md-6 -->
                    <div class="col-md-12">
                        @include('backend.product.includes.list_properties', ['properties' => $properties, 'productProperties' => $productProperties])
                        <div class="form-group row">
